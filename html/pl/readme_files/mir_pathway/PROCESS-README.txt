miRNA Pathway Analysis Pipeline
Author: Nick Bild
Date: 2016-11-08
...
The miRNA Pathway Analysis pipeline performs a pathway analysis via WebGestalt (http://bioinfo.vanderbilt.edu/webgestalt) on gene lists derived from a raw data file containing mature miRNA IDs, p-values, and fold changes.  Criteria for creating the gene lists are supplied by the pathway config file.  An HTML report for all analyses is provided.
...
Software installed on marburg (192.168.0.14) in /home/act/software/nickbild/mir_pathway.
...
http://192.168.0.16/ -> Pathway -> miRNA Web Gestalt Analysis
...
* How many databases must each miRNA/gene pair be found in to be retained?
 e.g.: 3
...
Sample names must be in the first column, and must match sample names in the header of the data file.
...
Links will also be provided to download each annotated miR/gene pair file.
...
* mir_pathway_run.pl - Creates filtered gene lists and annotated miR/gene pair files.

* mir_pathway_webgestalt - Runs the pathway analyses.
